...
$desayuno = current($conexion ->query("SELECT COUNT(idinvitados) FROM invitados WHERE comida_invitado='desayuno' AND fecha_inv = '$diahoytabla' ")->fetch_assoc() );

//Conbsulta SQL para las fechas (Almuerzo)
$almuerzo = current($conexion ->query("SELECT COUNT(idinvitados) FROM invitados WHERE comida_invitado='almuerzo' AND fecha_inv = '$diahoytabla' ")->fetch_assoc() );

//Conbsulta SQL para las fechas (cena)
$cena = current($conexion ->query("SELECT COUNT(idinvitados) FROM invitados WHERE comida_invitado='cena' AND fecha_inv = '$diahoytabla' ")->fetch_assoc() );
$total = current($conexion->query("SELECT COUNT(idinvitados) FROM invitados WHERE fecha_inv = '$diahoytabla'")->fetch_assoc() );
...
function Header()
{
    ...
    $this->Cell(50, 8, 'REPORTE DE TABLA DE COMIDA DE INVITADOS POR DIA',0,1);
...
$productos = (
    "SELECT *, UPPER(invitados.nombre_invi) AS nombre_mayusculas, UPPER(invitados.apellido_invi) 
    AS apellido_mayusculas FROM invitados 
    INNER JOIN usuario ON invitados.id_usuario_invi = usuario.idusuario WHERE fecha_inv = '$diahoytabla' ");
...
if ($resultado) {
    while ($row = mysqli_fetch_array($resultado)) {
        ...
        $pdf->Cell(32, 8, utf8_decode($row['fecha_inv']).'-'.$row['hora_inv'], 1, 0,'C', 0);
...
$pdf->SetX(159);
$pdf->Cell(50, 8, utf8_decode("TOTAL DE INVITADOS:"." ".$total), 1, 0,'C', 1);
