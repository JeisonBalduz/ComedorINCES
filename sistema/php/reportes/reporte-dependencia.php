...
<script src="../../js/libreriasJS/jquery-3.6.3.min.js"></script>
    ...
    <script type="text/javascript">
        Swal.fire({
            title: 'FALTO COLOCAR LA DEPENDENCIA!!',
            icon: 'error',
            confirmButtonText: false,
            showConfirmButton: false,
            footer:'<a href="../../reportes.php" class="boton-eliminar" id="boton-envio">Regresar</a>'
...
$total = current($conexion->query("SELECT COUNT(idticket) FROM perticket INNER JOIN personal ON perticket.idpersonal = personal.idpersonal INNER JOIN sedes ON personal.idsede = sedes.idsede WHERE sedes.sede= '$verSede' AND perticket.fecha BETWEEN '$verDesde' AND '$verHasta'   ")->fetch_assoc() );
...
function Header()
{
    ...
    $this->Cell(50, 8, 'REPORTE DE DEPENDENCIA',0,1);
    ...
    $this->Cell(25.2, 8, utf8_decode('CÉDULA'), 0, 0,'C', 1);
...
WHERE sedes.sede='$verSede' AND perticket.fecha BETWEEN  '$desde' AND '$hasta' ORDER BY perticket.fecha");
...
while ($row = mysqli_fetch_array($resultado)) {
    ...
    $pdf->Cell(35, 8, utf8_decode($row['fecha_español'])." ".utf8_decode($row['hora_ticket']), 1, 0,'C', 0);
...
$pdf->Output('reporte_dependencia.pdf', 'I');
?>
